...
#+END_QUOTE
*** 变量的设定规则
1. 变量和变量内容以一个等号来连接。如：myname=VBird
2. 等号两边不能直接接空格，如变量值中包含空格需要使用反斜杠（\)【转义字符：转义右边紧靠的那个字符】
#+BEGIN_QUOTE
name=VBird\'s\ name
#+END_QUOTE
3. 变量名只能是英文字母和数字
4. 在一串指令的执行中，还需要藉由其他额外的指令所提供的信息时，可以使用反单引号（`指令`）或（$(指令）)。
#+BEGIN_QUOTE
 version=$(uname -r)
 version=`uname -r`
#+END_QUOTE
5. 在变量中增加内容，可使用"$变量名称"或$(变量) 来累加内容。
#+BEGIN_QUOTE
PATH="$PATH":/home/bin  ---- 在PATH中增加 /home/bin
PATH=${PATH}:/home/bin 
name="$name"yes   ----在变量name中增加yes
#+END_QUOTE
6. 设置为环境变量（全局变量），使用export
#+BEGIN_QUOTE
export PATH
#+END_QUOTE
7. 通常大写字符为系统默认变量，自定义变量可以使用小写字符，方便判断。
8. 消除变量unset
#+BEGIN_QUOTE
unset myname
#+END_QUOTE
...
#+END_QUOTE
*** 环境变量的功能
环境变量可以帮我们达到很多功能~包括家目录的变换啊、提示字符的显示啊、执行文件搜寻的路径啊等等的, 还有很多很多啦!那么,既然环境变量有那么多的功能,问一下,目前我的 shell 环境中, 有多少默认的环境变量啊?我们可以利用两个指令来查阅,分别是 env 与 export 呢!用env（environment (环境) 的简写）可以列出系统中所有的环境变量。
**** 用env查看所有环境变量
***** 常见环境变量
- HOME
- SHELL
...
- LANG      --- 语系
- RANDOM    --- 随机数
**** 用set查看所有变量（包含环境变量和自定义变量）
一般来说,不论是否为环境变量,只要跟我们目前这个 shell 的操作接口有关的变量, 通常都会被设定为大写字符,也就是说,『基本上,在 Linux 预设的情况中,使用{大写的字母}来设定的变量一般为系统内定需要的变量』。
***** 一些重要的变量
- PS1 （提示字符的设定）
- $   （$$ 返回本shell的PID（线程代号））
- ？  （返回上一个指令的回传值）  
  当我们执行某些指令时, 这些指令都会回传一个执行后的代码。一般来说,如果成功的执行该指令,则会回传一个 0 值,如果执行过程发生错误,就会回传『错误代码』才对!一般就是以非为 0 的数值来取代。
**** export (自定义变量转为环境变量{相当于全局变量})
...
* 第十二章 正则表达式与文件格式化处理
** 语系对正则表达式的影响
- LANG=C时：0 1 2 4 …… A B C D …… Z a b c d …… z
- LANG=zh_CN时：0 1 2 3 4 …… a A b B c D …… z Z
使用正则表达式时，需要注意当时环境的语系，否则，可能会得到不同的结果。
** 正则表达式中的特殊符号

| 特殊符号   | 代表意义                                     |
| [:alnum:]  | 代表任何大小写字符和数字，即0-9，a-z，A-Z    |
| [:alpha:]  | 代表任何大小写字符                           |
| [:blank:]  | 代表空格和[tab]                              |
| [:cntrl:]  | 代表键盘上的控制按键，包括CR，LF，Tab，Del等 |
| [:digit:]  | 代表数字，0-9                                |
| [:graph:]  | 除了空格符（空格键和[tab]）外的所有按键      |
| [:lower:]  | 代表小写字符 a-z                             |
| [:upper:]  | 代表大写字符 A-Z                             |
| [:print:]  | 代表任何可以被打印的字符                     |
| [:punct:]  | 代表标点符号                                 |
...
** grep的一些高级参数
grep [-A] [-B] [--color=auto] '要寻找的字符串' filename
#+BEGIN_QUOTE
-A  后面可加数字，为after的意思，除了列出该行外，后续的n行也列出来；
-B  后面可加数字，为before的意思，除了列出该行外，前面的n行也列出来；
--color=auto  将正确的数据标示颜色
#+END_QUOTE
**  基础正则表达式字符
| RE字符 | 意义和范例 |
| ^word  | 意义：待查的字符串在行首 |
