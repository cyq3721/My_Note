...
** Lisp的数据类型
  ...
  二进制编码函数(byte-code function)，再加上一些特殊的类型，如buffer。同时，每种原始类型都有一个对应的函数去校验对象是否属于其类型。
** 符号类型(Symbols)
...
**** 属性列表
    ...
    注意，其中 *名字* 为字符串类型，不可改变，其他三个组成部分可被赋值为任意lisp对象。 其值为属性列表(plist)。一个以冒号开头的符号类型称之为keyword symbol，它常用于常量类型。
*** 定义符号类型
